...
// clayParamKeys -> Info.jsx rule display name, for the parameters that
...
const ReturnSandFoundrySandTestingNoteReport = () => {
  const { isAdmin, user, editWindowMs } = useAuth();
  const { toast } = useToast();
  const [showDeviations, setShowDeviations] = useState(false);
  const ruleByField = useMemo(() => {
    const map = {};
    returnSandValidationRanges.forEach((r) => { map[r.field] = r; });
    return map;
  }, []);
  ...
  const fetchSingleDate = async (date) => {
    ...
    setLoading(true);
    try {
      const url = `${API_ENDPOINTS.returnSandFoundrySandTestingNotes}?startDate=${encodeURIComponent(date)}&endDate=${encodeURIComponent(date)}`;
  ...
  const fetchRange = async (from, to) => {
    ...
    setShowEntryTable(false);
    try {
      const url = `${API_ENDPOINTS.returnSandFoundrySandTestingNotes}?startDate=${encodeURIComponent(from)}&endDate=${encodeURIComponent(to)}`;
      const res = await fetch(url, { credentials: 'include' });
      if (!res.ok) throw new Error(`Server error: ${res.status}`);
      const data = await res.json();
      const list = sortEntries((data.success && Array.isArray(data.data)) ? data.data : []);
  ...
  // A single-value cell (sieve): plain text in read mode, a text input wired
  // to the pending-edits map in edit mode.
  ...
  const handleSaveEdits = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_ENDPOINTS.returnSandFoundrySandTestingNotes}/${currentEntry._id}`, {
        method: 'PUT',
    ...
    if (editMode) {
      ...
      return (
        <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
          <input type="number" step="0.01" value={getEditValue('clayTests', testNum, `${param}.input1`, data?.input1)} onChange={(e) => setEditValue('clayTests', testNum, `${param}.input1`, e.target.value)} style={inputStyle} />
          <span>{operator}</span>
          <input type="number" step="0.01" value={getEditValue('clayTests', testNum, `${param}.input2`, data?.input2)} onChange={(e) => setEditValue('clayTests', testNum, `${param}.input2`, e.target.value)} style={inputStyle} />
          {!isSimple && (
            <>
              <span>/</span>
              <input type="number" step="0.01" value={getEditValue('clayTests', testNum, `${param}.input3`, data?.input3)} onChange={(e) => setEditValue('clayTests', testNum, `${param}.input3`, e.target.value)} style={inputStyle} />
            </>
          )}
        </div>
      );
  ...
  const summaryColumns = [
    ...
    { key: 'plant', label: 'Plant', width: '150px', align: 'center' }
  ];
  ...
  const handleExcelDownload = async ({ from: rawFrom, to: rawTo }) => {
    ...
    try {
      ...
      await exportWorkbookToExcel({
        title: 'Return Sand Foundry Sand Testing Note - Report',
        fromDate: from,
        toDate: to,
        fileName: 'Return_Sand_Foundry_Sand_Testing_Note_Report',
        sheets: [
  ...
  return (
    <div className="foundry-sand-testing-report-container">
      ...
      <div className="foundry-sand-testing-report-header">
        <div className="foundry-sand-testing-report-header-text">
          <h2>
            ...
            Return Sand Foundry Sand Testing Note - Report
          </h2>
        </div>
        {currentEntry && !showEntryTable && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap', fontWeight: 600, color: '#1e293b' }}>
            ...
            {currentEntry.plant && <span>Plant: {currentEntry.plant}</span>}
          </div>
        )}
      </div>
      ...
      {/* Combination table — pick a Date/Shift/Plant directly, or an empty table when no data is found */}
...
export default ReturnSandFoundrySandTestingNoteReport;
