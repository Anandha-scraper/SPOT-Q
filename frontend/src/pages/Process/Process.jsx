...
export default function ProcessControl() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        successMessage =
          data.message || "Process control entry saved successfully!";
      } else {
    ...
    try {
      ...
      const currentDate =
        dateData.success && dateData.data?.date
          ? dateData.data.date
          : formData.date;
    ...
    <>
      ...
      <div className="process-form-grid">
        ...
        {["C", "Si", "Mn", "P", "S", "MgFL", "Cu", "Cr"].map((el) => (
          <div className="process-form-group" key={el}>
            ...
            <input
              ref={(r) => (inputRefs.current[`metalComposition${el}`] = r)}
              type="number"
              name={`metalComposition${el}`}
              step="0.001"
              value={formData[`metalComposition${el}`]}
              onChange={handleChange}
              onKeyDown={(e) => handleKeyDown(e, `metalComposition${el}`)}
              placeholder="%"
            />
          </div>
        ))}
        ...
        <div className="process-form-group">
          ...
          <input
            ref={(el) => (inputRefs.current.timeOfPouring = el)}
            type="time"
            name="timeOfPouring"
            value={formData.timeOfPouring}
            onChange={handleChange}
            onKeyDown={(e) => handleKeyDown(e, "timeOfPouring")}
          />
        </div>
        ...
        <div className="process-form-group">
          ...
          <input
            ...
            onChange={handleChange}
            onKeyDown={(e) => handleKeyDown(e, "tappingTime")}
          />
        </div>
        ...
        {["C", "Si", "Mn", "S", "Cr", "Cu", "Sn"].map((el) => (
          <div className="process-form-group" key={`add-${el}`}>
            <label>{el}</label>
            <input
              ref={(r) => (inputRefs.current[`correctiveAddition${el}`] = r)}
              type="number"
              name={`correctiveAddition${el}`}
              step="0.01"
              value={formData[`correctiveAddition${el}`]}
              onChange={handleChange}
              onKeyDown={(e) => handleKeyDown(e, `correctiveAddition${el}`)}
              placeholder="Kgs"
            />
          </div>
        ))}
      ...
      <div className="process-submit-container">
        ...
        <button
          ...
          title={
            !isPrimarySaved ? "Please save Primary data first" : "Submit Entry"
          }
        >
          {submitLoading ? (
